...
export function SiteFooter() {
  return (
    <footer className="bg-[#0a0a2e] text-heroink-100">
      {/* Top accent line */}
        ...
        <div className="lg:col-span-2">
          ...
          <ul className="mt-6 space-y-2.5 text-sm text-white/55">
            <li className="flex items-center gap-2.5">
              <MapPin size={15} className="text-indigo-400" /> طرابلس، ليبيا
            </li>
            <li className="flex items-center gap-2.5">
            ...
            <li className="flex items-center gap-2.5">
              <Mail size={15} className="text-indigo-400" /> info@alphabeta.ly
            </li>
          </ul>
        </div>
        ...
        {COLUMNS.map((col) => (
          <div key={col.title}>
            ...
            <ul className="space-y-2.5">
              {col.links.map((l) => (
                <li key={l.to + l.label}>
                  <Link to={l.to} className="text-sm text-white/50 transition-colors hover:text-indigo-300">
                    {l.label}
                  </Link>
                </li>
              ))}
      ...
      <div className="border-t border-white/8">
        <div className="mx-auto flex w-full max-w-7xl flex-wrap items-center justify-between gap-3 px-5 py-5 text-xs text-white/35 sm:px-8">
          <p>© {new Date().getFullYear()} ألفا بيتا — جميع الحقوق محفوظة</p>
          <p className="flex gap-4">
            <Link to="/contact" className="transition-colors hover:text-indigo-300">تواصل معنا</Link>
            <Link to="/request-quote" className="transition-colors hover:text-indigo-300">اطلب عرض سعر</Link>
          </p>
        </div>
      </div>
    </footer>
